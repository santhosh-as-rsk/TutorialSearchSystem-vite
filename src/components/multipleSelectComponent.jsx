...
export function MultiSelectTopicsSearchBar() {
    ...
    const fetchTopics = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/fetchTopics"
        );
  ...
  const fetchSubTopics = async (selectedTopics) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/fetchMultipleSubTopics?topicNames=${selectedTopics}`
      );
      const data = response.data;
  ...
  const fetchTableData = async (page = 1, perPage = 10) => {
    if (selectedTopics && selectedSubTopics && page && perPage) {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/api/fetchTutorialLinkMultipleSubTopics?subTopicNames=${selectedSubTopics}&page=${adjustPage}&size=${perPage}`
        );
  ...
  const submitOnclick = async () => {
    if (selectedTopics.length > 0 && selectedSubTopics.length > 0) {
      fetchTableData();
    } else alert("please select Topic and Sub-Topic");
  };
  ...
  const handleSearch = (changedvalue) => {
    ...
    const filteredLinks = tutorialLink.filter(
      (link) =>
        (link.links &&
          link.links.toLowerCase().indexOf(changedvalue.toLowerCase()) !==
            -1) ||
        (link.subTopicsId.name &&
          link.subTopicsId.name
            .toLowerCase()
            .indexOf(changedvalue.toLowerCase()) !== -1) ||
        (link.subTopicsId.topicsId.name &&
          link.subTopicsId.topicsId.name
            .toLowerCase()
            .indexOf(changedvalue.toLowerCase()) !== -1) ||
        (link.sourceId &&
          link.sourceId.name &&
          link.sourceId.name.toLowerCase().indexOf(changedvalue.toLowerCase) !==
            -1)
    );
  ...
  return (
    <React.Fragment style={{ outerWidth: "100%" }}>
        ...
        <Row className="justify-content-center">
          ...
          <Col xs={12} md={6} lg={4} className="mb-3 mx-auto">
            <AutocompleteMultipleSelect
              ...
              labelName="Sub Topics"
            />
          </Col>

          <Col xs={12} md={6} lg={1} className="mb-3 text-md-end">
            <Button variant="success" onClick={submitOnclick}>
              submit
            </Button>
          </Col>
        </Row>
